...
const FilesList = () => {
  ...
  const getFilesList = async () => {
    try {
      const { data } = await axios.get('https://upload-file-lad3.onrender.com/api/getAllFiles');
      setErrorMsg('');
      setFilesList(data);
    } catch (error) {
  ...
  const downloadFile = async (id, path, mimetype) => {
    try {
      const result = await axios.get(`https://upload-file-lad3.onrender.com/api/download/${id}`,{
        responseType: 'blob'
      });
